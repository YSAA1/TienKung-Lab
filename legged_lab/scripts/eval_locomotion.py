...
parser = argparse.ArgumentParser(description="Evaluate locomotion terrain progress.")
...
def evaluate() -> dict:  # noqa: C901 -- existing sequential evaluation pipeline
    ...
    scan_dim = env.observation_layout.scan_dim
    scan_end = scan_start + scan_dim * scan_history_length
    ...
    scan_permutation = torch.randperm(scan_dim, generator=permutation_generator).to(env.device)

    def apply_scan_mode(policy_obs: torch.Tensor, mode: str) -> torch.Tensor:
        if mode == "normal":
            return policy_obs
        transformed = policy_obs.clone()
        scan = transformed[:, scan_start:scan_end].reshape(transformed.shape[0], scan_history_length, scan_dim)
        if mode == "zero":
            scan.zero_()
        elif mode == "permuted":
    ...
    result = {
        "evaluator": "locomotion_terrain_perception_v4",
        "checkpoint": str(root),
        "task": args_cli.task,
