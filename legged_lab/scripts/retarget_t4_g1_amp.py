...
"""Retarget the actual T4 forward walk/jog source clips to the official G1 29DoF.
    ...
    def convert(self, rows):
        ...
        for row in rows:
            ...
            solution = least_squares(
                ...
                bounds=(self.bounds[self.active, 0] + 1e-5, self.bounds[self.active, 1] - 1e-5),
                max_nfev=60,
                gtol=1e-7,
                ftol=1e-7,
                xtol=1e-7,
...
def main():
    ...
    prepared = []
    manifest = {
        "source_dataset": "T4 runtime forward walk/jog CSV robot-to-robot retarget",
        ...
        "limitations": "Kinematic mapping; no claim of dynamic tracking or trained locomotion. Two short source clips.",
        "sources": {},
        "clips": {},
    }
    for stem, cls in (("t4_walk_forward", "walk_forward"), ("t4_jog_forward", "run")):
        ...
        stats.update(
            ...
            source_stop=stop,
            source=stem,
            full_stride_cycles=len(peaks) - 1,
    ...
    for stem, rows, kin, metrics, stats in prepared:
        ...
        path = out / f"{name}.txt"
        weight = manifest["class_probabilities"][stats["motion_class"]]
        payload = {
            "LoopMode": "Clamp",
            "FrameDuration": 1 / 30,
            "MotionWeight": weight,
    ...
    manifest["duration_by_class_s"] = {s["motion_class"]: s["duration_s"] for _, _, _, _, s in prepared}
