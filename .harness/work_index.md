...
| G1 视觉学生主线 + 教师巩固 | active | B1 done（runbook）；A2 done（vital_v31 三项 DR）；B3 部分（dropout 调档）；**B2 代码落地 2026-09-12**（G1 学生 contract/env/配方/入口 + 4 项合同测试，Isaac 实启待 B4）。待办：A1 v3 验收（16k/30k 在训）、A3 重训、B4/B5。G1 sim2sim 学生脚本挂 B4 | `docs/plans/2026-09-11--g1-vision-student-and-teacher-v31-plan.md` |
| Z2 AMP curated v2 + vital_v31 | active | 复查发现 v1 run/walk 为原地踏步（净位移 ~1cm）→ curated v2 = {walk_l, run2, run_l, run_140_l}（全前进 ≥0.76 m/s，上游 fixture 晋升）；plant DR 抽共享层 `locomotion/plant_dr.py` + `z2_vital_v31` 入口；v1 lineage 与 `z2_loco_teacher` 默认零变化。expert 生成（Isaac）与 30k 重训走 nubot bundle 链路（GitHub 直连慢），启动脚本 `scripts/train_z2_vital_v31.sh`（GPU1/3）。复查证据 `artifacts/z2_amp_recheck_20260912/` | 提交 `e1192f1`；`docs/plans/`（并入主计划 Z2 行） |
