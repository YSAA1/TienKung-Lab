...
- Parallel surface: Stage E teacher（`docs/plans/2026-08-12--t4-unified-depth-locomotion-plan.md`，M1/M2 与 `stage_e_prov6_local` 本机训练中；prov5 爬行作弊已作废，见下方 lineage 记录）
...
- GPU 分配裁定（2026-08-14 修订）: prov7 用 nubot 2 卡（CUDA 0,1）×2048 env；nubot 其余 2 卡留空；跨栏侧取消 zhuoqun 2 卡配额（原 2026-08-13 的 2+2 裁定中 vault 是否回收 4 卡由 vault surface 裁定）；本机 1 卡渲染/调试。
...
- Running lineage: `stage_e_prov6_local`（commit `859fe24` 姿态摔倒终止修复，从零训练，本机 tmux `t4-stage-e-local`，1x RTX 4090，1024 env，40000 iter 预算，日志 `logs/t4-stage-e-teacher.stage_e_prov6_local.log`，TensorBoard tmux `t4-tb-local` 端口 6008，PYTHONPATH 需含仓库根与 `rsl_rl/`——本机 env_isaaclab 的 site-packages 有 Z2-Lab-stable 的 rsl_rl editable 安装会抢占）。启动于 2026-08-13 21:17，iter ~200 时 ~16.3k steps/s、显存 9.1/24.5GB，`termination_penalty` 生效（mean episode length ~112），无 NaN。
