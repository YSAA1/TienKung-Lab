...
- 任务 `t4_loco_teacher_sparse`。现行 **S12 热启** worktree `/home/nubot/phn_ws/t4_train/TienKung-Lab-s12-from-s11b-5k`，run `t_sparse_lightlp_s12_from_s11b_5k`，从 S11b `model_19000` 加载、`--reset_optimizer`、再训 5000 到 24000。tmux `t4-sparse-lightlp-s12-from-s11b-5k`，TB `:8016`。
- 配方：0.75 m 收尾边框 + 踏石/圆桩 40% 轻转；`vx` 非洞 `[-0.6, 2.0]`，洞上 `[0.6, 2.0]`。终止阈值不改。
- 学生 waiter `t4-s12-gru-student-wait` 在 `model_23999.pt` 落盘后自动开 `t4-sparse-s12-gru-student`。不 resume `stage_s_head35`。
