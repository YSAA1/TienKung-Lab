...
def train():
    ...
    log_root_path = "artifacts/portability/v4/ddp_probe"
    ...
    aux_ids, _ = env.robot.find_joints(list(env.cfg.robot_spec.auxiliary_joint_names), preserve_order=True)
    assert env.num_actions == 29 and env.robot.num_joints == 43
    assert torch.equal(env.action[:, aux_ids], torch.zeros_like(env.action[:, aux_ids]))
    assert torch.equal(env.robot.data.joint_pos_target[:, aux_ids], env.robot.data.default_joint_pos[:, aux_ids])
    ...
    result["auxiliary_default_targets_verified"] = True
    result["auxiliary_max_abs_velocity"] = float(env.robot.data.joint_vel[:, aux_ids].abs().max())
    ...
    result["asset_path"] = env.cfg.scene.robot.spawn.usd_path
    Path("artifacts/portability/v4/runtime_rank%d.json" % dist.get_rank()).write_text(json.dumps(result, indent=2)+"\n")
