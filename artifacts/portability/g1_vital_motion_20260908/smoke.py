...
OUTPUT = Path("artifacts/portability/g1_vital_motion_20260908/smoke")
...
def checked_learn(self, num_learning_iterations, init_at_random_ep_len=False):
    ...
    assert tuple(cfg.deterministic_fall_limits) == (0.8, 1.0)
    ...
    assert cfg.reward.velocity_slack.weight == 1.5
    assert cfg.reward.track_ang_vel_z_exp.weight == 2.0
    assert cfg.reward.body_orientation_l2.weight == 0.0
