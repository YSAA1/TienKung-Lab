...
"""Paired cold training: the only task difference is reset root velocity x/y."""
...
parser.add_argument("--group", choices=("control", "random_xy"), required=True)
...
# Preserve the launched import order after AppLauncher initialization.
# isort: off
import numpy as np
import torch
...
def main():
    ...
    if args.group == "control":
        ...
        assert float(velocity[:, :2].abs().max()) <= 0.50001
        assert bool((velocity[:, :2].std(0) > 0.2).all())
