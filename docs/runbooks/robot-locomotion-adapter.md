...
| AMP 数据合同 | `assets/<robot>/schemas.py` + `scripts/generate_<robot>_amp_expert.py` | 70D AMP 帧布局（`AMP_FIELDS`）、专家目录常量与逐段 sha256 manifest；专家生成与运行时共用 `AmpFeatureBuilder`。数据集文本一律 LF（`.gitattributes` 已强制），manifest 哈希按 LF 字节记录。上游溯源 fixture 用一次性脚本拉取并 pin 提交（参考 `scripts/fetch_z2_upstream_fixtures.py`），不整仓 vendor。 |
...
| 公共训练/评估 | `scripts/train.py`、`scripts/eval_locomotion.py`、`scripts/play.py` | 使用 `--task` 选择机器人，不能加入机器人名字分支。任务注册表返回深拷贝配置，脚本内改参不会污染注册表 |
