...
def _third_robot():
    ...
    return LocomotionRobotSpec(
        ...
        hands=("tip_a", "tip_b"),
        torso="chassis",
        diagnostic_bodies=("chassis",),
        left_leg=joints[:6],
        right_leg=joints[6:12],
        ankles=(joints[4], joints[10], joints[5], joints[11]),
        hand_site_offset=(0, 0, 0),
        foot_site_offset=(0, 0, 0),
        mirror_indices=tuple(range(6, 12)) + tuple(range(6)) + tuple(range(12, 21)),
        mirror_signs=(-1, 1, -1, 1, 1, -1) * 2 + (1,) * 9,
        nominal_feet_distance=0.18,
        reward_bodies={},
        reward_joints={},
    )
...
def _articulation(spec):
    ...
    data = SimpleNamespace(
        ...
        body_quat_w=torch.tensor([1.0, 0.0, 0.0, 0.0]).repeat(1, b, 1),
    )
...
@pytest.mark.parametrize(
    "spec,actor,critic,amp",
    [
        (T4_LOCOMOTION, 1937, 2016, 66),
        (G1_LOCOMOTION, 1997, 2076, 70),
        (Z2_LOCOMOTION, 1997, 2076, 70),
        (_third_robot(), 1757, 1836, 54),
    ],
)
def test_layout_amp_and_mirror_use_robot_contract(spec, actor, critic, amp):
    spec.validate()
    layout = ObservationLayout(
        len(spec.joint_names), scan_history=5, actor_contact=True, critic_foot_scan=True, critic_immunity=True
    )
...
def test_runtime_rejects_wrong_robot_and_training_contract():
    ...
    env = SimpleNamespace(
        cfg=SimpleNamespace(robot_spec=spec),
        robot=_articulation(spec),
        observation_layout=layout,
        get_observations=lambda: (
            torch.zeros(1, layout.actor_dim),
...
def test_amp_sites_rotate_in_body_then_root_frame():
    yaw90 = torch.tensor([[2**-0.5, 0.0, 0.0, 2**-0.5]])
    result = site_pos_in_root_frame(
    ...
    assert torch.allclose(result, torch.tensor([[2.0, 0.0, 0.0]]), atol=1e-6)
